...
///pickerView
@property (nonatomic, weak) id datePickerView;
///时间回调
...
+ (instancetype)sharePickerViewWithCurrentDate:(NSString *)currentDate isMax:(BOOL)ismax DateBlock:(void (^)(NSString *))dateBlock {
...
- (void)czh_setDate {
    ...
    if(self.ismax){
          ((UIDatePicker *)self.datePickerView).maximumDate = todatdate;
    }else{
         ((UIDatePicker *)self.datePickerView).minimumDate = todatdate;
    }
   
    
    if (self.currentDate.length <=0) {//如果没有初始时间用当前时间
        ((UIDatePicker *)self.datePickerView).date = todatdate;
    } else {//用传入的时间代替
        NSDateFormatter *dateFormatter = [[NSDateFormatter alloc] init];
        [dateFormatter setDateFormat:FORMAT];
        NSDate *date = [dateFormatter dateFromString:self.currentDate];
         ((UIDatePicker *)self.datePickerView).date = date;
    }
}
...
- (void)buttonClick:(UIButton *)sender {
    ...
    if (sender.tag == CZHDatePickerViewButtonTypeSure) {
        ...
        NSDateFormatter *dateFormatter = [[NSDateFormatter alloc] init];
        [dateFormatter setDateFormat:FORMAT];
        NSString *strDate = [dateFormatter stringFromDate: ((UIDatePicker *)self.datePickerView).date];
